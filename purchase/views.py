    ...
    try:
        ...
        if order:
            ...
            for item in cart_items:
                ...
                purchased_item.variation_id = item.variation_id
                purchased_item.save()

                if purchased_item.variation.is_available:
                    purchased_item.save()
                    item.delete()
                else:
...
def accept_order(request, order_key):
    if not request.user or not request.user.is_authenticated:
        return redirect('login')
    if not order_key:
        return redirect('cart')  # TODO: Add message div for error here
...
def reserve_order(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            receipt = Receipt(owner=request.user,
